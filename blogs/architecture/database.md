...
Databases show up in nearly every system: [PostgreSQL](https://www.postgresql.org/) through [Supabase](https://supabase.com/), [MongoDB](https://www.mongodb.com/) for document stores, [Redis](https://redis.io/) for caching, and [Qdrant](https://qdrant.tech/) for vector search. The API surface is usually easy to learn first. The harder part is understanding how the storage engine, indexing, and persistence pieces work underneath.
...
Instead of reading and writing data to disk, you keep everything in RAM. A RAM access is around 100ns. A random read on a fast NVMe SSD is somewhere between 100μs and 1ms, a factor of $10^3$ to $10^4$ difference, and for workloads doing millions of operations per second, that gap is the entire ballgame.
...
The vector database work came from building RAG systems in practice. Qdrant was the default tool for a lot of those projects, and it made the core tradeoffs clear: the system needs to ingest data, index it well, and return relevant results quickly enough to be useful in an application.

That led to a broader question: what if the database itself handled more of the retrieval work intelligently, instead of leaving every decision to application code? Index selection, search tuning, and retrieval behavior are all part of that problem.

It is useful to think of a vector database as external retrieval memory for AI applications. It is a storage-layer version of the same basic idea that transformers use internally with attention and embeddings.
...
A complete vector database involves several parts that have to work together: ingesting data, organizing it for fast retrieval, and answering queries in a way that is both fast and accurate. The architecture is tightly coupled, so each component affects the others.

#### The Embedding Model

The first step happens before the database even sees the data. An embedding model processes raw input such as text, images, or audio and outputs a fixed-length vector. The model is trained so semantically similar inputs produce nearby vectors in high-dimensional space. The database never works directly with the raw input; it only works with these vectors, and the meaning depends entirely on the model that produced them. If the model changes, the dataset has to be re-embedded because the vector space changes too.
...
The query engine ties everything together. It receives the incoming request, decides which index to use, coordinates the ANN search with metadata filtering, applies the chosen distance metric, and returns the top $k$ results ranked by similarity score. It also handles concurrency and enforces budget constraints like timeouts or complexity limits.

How these pieces interact at the query layer has a bigger impact on real-world latency than most people expect. The index type, the distance metric, the filter strategy, whether vectors are memory-mapped or fully loaded, and whether metadata lives inline or in a separate store all compound at query time. A well-tuned query engine can feel instant. A poorly configured one on the same hardware can be 10–100× slower.

> More on these things in [later sections](/blogs/architecture)

### What does the flow look like in practice?

Putting it all together: raw text goes into an embedding model, which outputs a vector. That vector and its metadata are stored in the database, and the index is updated to place it in the right neighborhood. When a user asks a question, the question is converted into a vector by the same model. The database then runs ANN search, applies any metadata filters, and returns the top $k$ semantically closest results.

The rest of these [architecture posts](/blogs/architecture) go through each component in detail.
